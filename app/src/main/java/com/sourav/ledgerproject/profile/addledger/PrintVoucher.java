...
public class PrintVoucher {
    ...
    private List<VoucherLedger> voucher_list;
    boolean payment;
    ...
    private Map<String,Integer> getTotalDebitOrCredit(){
        int totaldebit=0;
        int totalcredit=0;
        for(VoucherLedger debit:voucher_list){
            if(debit.getVoucher_mode().equals("Payment")){
                totaldebit+=Integer.parseInt(debit.getAmount());
                Log.d(TAG,"total debit: "+totaldebit);
            }
            else if(debit.getVoucher_mode().equals("Receipt")){
    ...
    public void setVoucher_list(List<VoucherLedger> voucher_list){
    ...
    public List<VoucherLedger> getVoucher_list() {
        return voucher_list;
    }
    ...
    private void createPage(){
        ...
        for(VoucherLedger vo: getVoucher_list()){

            if(vo.getVoucher_mode().equals("Payment")) {
                voucher_p = vo.getCreation_time() + "                        " + vo.getVoucher_mode() + "                     " + vo.getAmount();
                payment = true;
            }
            else if(vo.getVoucher_mode().equals("Receipt"))
                voucher_p = vo.getCreation_time()+"                        "+vo.getVoucher_mode()+"                                              "+vo.getAmount();
